...
public class TestScenarioP5 extends Robot {
	...
	public void LineMovement(String color) {
		...
		if (this.getTouch().isPressed()) {	//Si le capteur tactile est touche:
			...
			this.getInfo().setExistePalet(false);	//La valeur de existe palet = false pour indiquer au robot qu'il ya plus de palets sur la ligne
		}
	}
	...
	public void MainMethod(String color,String direction) {
		this.getRoues().MoveForwardSpeed(200f); //Le robot avance
		do {
	...
	public void BiggerMethod(String color,String direction) {
		while (Button.ESCAPE.isUp() && this.getInfo().getExistePalet()) { //Si existePalet = true on repete la boucle
			...
			if (this.getInfo().getExistePalet()) { //Dans le cas ou on attrape un palet (La rencontre de mur met la valeur de ExistePalet = false)
	...
	public static void main (String[] args) {
		...
		if (robot.getInfo().getCampDep()=="Ouest") {
			robot.getRoues().MoveForwardSpeed(200f); //Robot avance
			do {
			...
			if (robot.getColor().getCouleurCourant()=="YELLOW"){ //Si le couleur renconter = jaune
				...
				robot.getRoues().MoveForwardSpeed(200f);//Et avance
				do {
		...
		robot.getInfo().setExistePalet(true);//Apres avoir parcouru toute la ligne, ExistePalet devient false.
		...
		if (robot.getInfo().getExistePalet()) {//Si un palet existe(le robot a toucher un palet et n'a toujours pas detecte le mur)
			robot.StopWhite();//Le robot amene le palet vers le camp oppose.
		}
		if (robot.getInfo().getExistePalet()) {//Petite modification pour le 2eme iteration de MainMethod.
			robot.MainMethod("BLACK", SecondDir);//2eme iteration de MainMethod.
			if (robot.getInfo().getExistePalet()) {
				robot.StopWhite();
			}
		}
		if (robot.getInfo().getExistePalet()) {//Petite modification pour le 3eme iteration de MainMethod.
			...
			else if (SecondDir=="Left") {//La idee mais que le if d'avant mais si SecondDir == "Left".
		...
		if (robot.getInfo().getExistePalet()) {//Derniere et 4e iteration de MainMethod.
			robot.MainMethod("BLACK", SecondDir);
			if (robot.getInfo().getExistePalet()) {
				robot.StopWhite();
			}
		}
		robot.getInfo().setExistePalet(true);//Apres avoir parcouru toute la ligne, on remet ExistePalet a true pour executer le prochain BiggerMethod
